import "./Projects.css";
import car from "../assets/carprize.jpg";
import node from "../assets/node.svg";
import Html from "../assets/css.svg";
import css from "../assets/css-3-svgrepo-com.f1823d8606e24164d5ac03b4e9b80d85.svg";
import react from "../assets/react.svg";
import mong from "../assets/MongoDB_Logomark_ForestGreen.png";
import { FaGithubAlt } from "react-icons/fa";
import fb from "../assets/fb.svg";
import climate from "../assets/climate.jpg";  
import auth from "../assets/auth.webp";
import truck from "../assets/ashok.jpg";

const Project = () => {
  const titles = [
    "Carz",
    "Truck Inventory",
    "",
    "",
    "climatey",
    "Aroundly",
  ];
  const desc = [
    "A NEXT.js application utilizes an API to retrieve and display the prices of cars. The application likely features a frontend built with NEXT.js, a React framework for server-side rendering and client-side rendering.",
    "Devoloped a role based authentication using Mongodb express nodejs and reactjs for restricting the users from having access to the admin roled content",
    "Designed a web app for ashok leyland which is used for providing the users the spare parts only when there is a damage in the spare part in that truck",
    "Climate detector using API , Which takes the input such as location and return the Humidity and Wind Speed etc",
    ""
  ]
  return (
    <div className="main-div" id="projects">
      <div
        style={{
          display: "flex",
          justifyContent: "center",
          paddingTop: "1rem",
          textAlign: "center",
        }}>
        <p style={{ fontSize: "3rem", fontWeight: "bold" }}>
          My Recent <span className="purple"> Works</span>
        </p>
      </div>

      <div className="projects-col">
        <div className="cards">
          <div className="card">
            <div style={{ margin: "1rem" }}>
              <div
                style={{
                  display: "flex",
                  justifyContent: "center",
                  margin: "10px",
                }}>
                <img
                  src={car}
                  alt=""
                  style={{ width: "100%", height: "10rem" }}
                />
              </div>
              <div>
                <h2
                  style={{
                    fontSize: "2em",
                    justifyContent: "center",
                    display: "flex",
                  }}>
                  {titles[0]}
                </h2>
                <h2
                  style={{
                    fontSize: "1em",
                    justifyContent: "center",
                    display: "flex",
                  }}>
                  {desc[0]}
                </h2>
              </div>
              <div className="tech-stack">
                <p style={{ display: "flex", justifyContent: "center" }}>
                  Tech Stack
                </p>
                <div className="boxx">
                  <button type="button" className="btn btn-primary">
                    <img src={Html} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className="btn btn-primary">
                    <img src={css} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className=" btn-primary">
                    <img src={react} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className=" btn-primary">
                    <img src={node} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className=" btn-primary">
                    <img
                      src={mong}
                      style={{ height: "50px" }}
                      className="btn-image"
                      alt="nodeimg"
                    />
                  </button>
                </div>
              </div>
              <div className="onlinebtn">
                <div className="buttonss">
                  <a href="https://github.com/Pranaveshit/bits_pilani">
                    {" "}
                    <button>
                      {" "}
                      <FaGithubAlt /> Github
                    </button>
                  </a>
                </div>
                <div className="buttonss"></div>
              </div>
            </div>
          </div>
        </div>

        <div className="cards">
          <div className="card">
            <div style={{ margin: "1rem" }}>
              <div
                style={{
                  display: "flex",
                  justifyContent: "center",
                  margin: "10px",
                }}>
                <img
                  src={truck}
                  alt=""
                  style={{ width: "100%", height: "10rem" }}
                />
              </div>
              <div>
                <h2
                  style={{
                    fontSize: "2em",
                    justifyContent: "center",
                    display: "flex",
                  }}>
                  {titles[1]}
                </h2>
                <h2
                  style={{
                    fontSize: "1em",
                    justifyContent: "center",
                    display: "flex",
                  }}>
                  {desc[2]}
                </h2>
              </div>
              <div className="tech-stack">
                <p style={{ display: "flex", justifyContent: "center" }}>
                  Tech Stack
                </p>
                <div className="boxx">
                  <button type="button" className=" btn-primary">
                    <img src={react} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className=" btn-primary">
                    <img src={fb} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className=" btn-primary">
                    <img
                      src="https://th.bing.com/th?id=OIP.LOZctQTRjEnOcFUtVBaj-AHaDm&w=350&h=169&c=8&rs=1&qlt=90&o=6&dpr=1.5&pid=3.1&rm=2"
                      className="btn-image"
                      alt="nodeimg"
                    />
                  </button>
                  <button type="button" className=" btn-primary">
                    <img
                      src="                                    data:image/jpeg;base64,"
                      className="btn-image"
                      alt="nodeimg"
                    />
                  </button>
                </div>
              </div>
              <div className="onlinebtn">
                <div className="buttonss">
                  <a href="https://github.com/Pranaveshit/IT-Elites_Codebreak">
                    <button>
                      {" "}
                      <FaGithubAlt /> Github
                    </button>
                  </a>
                </div>
                <div className="buttonss"></div>
              </div>
            </div>
          </div>
        </div>
        <div className="cards">
          <div className="card">
            <div style={{ margin: "1rem" }}>
              <div
                style={{
                  display: "flex",
                  justifyContent: "center",
                  margin: "10px",
                }}>
                <img
                  src={climate}
                  alt=""
                  style={{ width: "100%", height: "10rem" }}
                />
              </div>
              <div>
                <h2
                  style={{
                    fontSize: "2em",
                    justifyContent: "center",
                    display: "flex",
                  }}>
                  {titles[4]}
                </h2>
                <h2
                  style={{
                    fontSize: "1em",
                    justifyContent: "center",
                    display: "flex",
                  }}>
                  {desc[3]}
                </h2>
              </div>
              <div className="tech-stack">
                <p style={{ display: "flex", justifyContent: "center" }}>
                  Tech Stack
                </p>
                <div className="boxx">
                  <button type="button" className=" btn-primary">
                    <img src={react} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className=" btn-primary">
                    <img src={fb} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className=" btn-primary">
                    <img
                      src="https://th.bing.com/th?id=OIP.LOZctQTRjEnOcFUtVBaj-AHaDm&w=350&h=169&c=8&rs=1&qlt=90&o=6&dpr=1.5&pid=3.1&rm=2"
                      className="btn-image"
                      alt="nodeimg"
                    />
                  </button>
                </div>
              </div>
              <div className="onlinebtn">
                <div className="buttonss">
                  <a href="https://github.com/Pranaveshit/omnify">
                    <button>
                      {" "}
                      <FaGithubAlt /> Github
                    </button>
                  </a>
                </div>
                <div className="buttonss"></div>
              </div>
            </div>
          </div>
        </div>
       
        <div className="cards">
          <div className="card">
            <div style={{ margin: "1rem" }}>
              <div
                style={{
                  display: "flex",
                  justifyContent: "center",
                  margin: "10px",
                }}>
                <img
                  src={auth}
                  alt=""
                  style={{ width: "100%", height: "10rem" }}
                />
              </div>
              <div>
                <h2
                  style={{
                    fontSize: "2em",
                    justifyContent: "center",
                    display: "flex",
                  }}>
                  {titles[5]}
                </h2>
                <h2
                  style={{
                    fontSize: "1em",
                    justifyContent: "center",
                    display: "flex",
                  }}>
                  {desc[1]}
                </h2>
              </div>
              <div className="tech-stack">
                <p style={{ display: "flex", justifyContent: "center" }}>
                  Tech Stack
                </p>
                <div className="boxx">
                  <button type="button" className="btn btn-primary">
                    <img src={Html} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className="btn btn-primary">
                    <img src={css} className="btn-image" alt="nodeimg" />
                  </button>
                  <button type="button" className=" btn-primary">
                    <img src={react} className="btn-image" alt="nodeimg" />
                  </button>

                  <button type="button" className=" btn-primary">
                    <img src={fb} className="btn-image" alt="nodeimg" />
                  </button>
                </div>
              </div>
              <div className="onlinebtn">
                <div className="buttonss">
                  <a href="https://github.com/Pranaveshit/Aroundlyintern">
                    <button>
                      {" "}
                      <FaGithubAlt /> Github
                    </button>
                  </a>
                </div>
                <div className="buttonss"></div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
export default Project;
